...
export default function ClientDetailsHeader({client}: Props) {
    ...
    return (
        <Card sx={{ position: 'relative', mb: 2, backgroundColor: 'transparent', overflow: 'hidden' }}>
            ...
            <CardMedia
                component="img"
                height="300"
                image={`/images/categoryImages/${client.firstName}.jpg`}
                alt={`${client.firstName} image`}
            />
            <Box sx={{
                position: 'absolute',
                bottom: 0,
                    ...
                    {isHost ? (
                        <>
                            <Button
                                ...
                                onClick={() => { }}
                            >
                                {isCancelled ? 'Re-activate client' : 'Cancel client'}
                            </Button>
                            <Button
                                variant="contained"
                                color="primary"
                        ...
                        >
                            {isGoing ? 'Cancel Attendance' : 'Join client'}
                        </Button>
                    )}
                </Box>
            </Box>
